...
let process_connection_fds store cons domains rset wset spec_fds =
  ...
  let process_fdset_with fds fct =
    List.iter
      (fun fd ->
        try try_fct fct (Connections.find cons fd) with Not_found -> ()
      )
      fds
  in
  ...
  try
    let channel =
      open_out_gen
        ...
        0o600
        (Paths.xen_run_stored ^ "/db.debug")
    in
    finally (fun () -> Store.dump store channel) (fun () -> close_out channel)
  with _ -> ()
...
let parse_config ?(strict = false) filename =
  ...
  let options =
    ...
    ; ( "conflict-max-history-seconds"
      , Config.Set_float Define.conflict_max_history_seconds
      )
    ; ( "conflict-rate-limit-is-aggregate"
      , Config.Set_bool Define.conflict_rate_limit_is_aggregate
      )
    ...
    ; ( "access-log-transactions-ops"
      , Config.Set_bool Logging.access_log_transaction_ops
      )
  ...
  in
  ( try Config.read filename options (fun _ _ -> raise Not_found) with
  | Config.Error err as e ->
      List.iter
        (fun (k, e) ->
          match e with
          | "unknown key" ->
              eprintf "config: unknown key %s\n" k
          | _ ->
              eprintf "config: %s: %s\n" k e
        )
        err ;
...
module DB = struct
  ...
  let from_channel_f chan global_f evtchn_f socket_f domain_f watch_f store_f =
    ...
    let getpath s =
      ...
      debug "Path: %s" u ; Store.Path.of_string u
    in
    ...
    while not !quit do
      try
        ...
        try
          ...
          | "dom" :: domid :: mfn :: remote_port :: rest ->
              let local_port =
                ...
                | local_port :: _ ->
                    Some (Utils.int_of_string_exn local_port)
              in
              domain_f ?local_port
        ...
        with exn ->
          warn "restoring: ignoring unknown line: %s (exception: %s)" line
            (Printexc.to_string exn) ;
          ()
      with End_of_file -> quit := true
    done ;
  ...
  let from_channel store cons domains_init chan =
    ...
    let socket_f ~fd =
      ...
      let is_valid =
        try (Unix.fstat ufd).Unix.st_kind = Unix.S_SOCK with _ -> false
      in
      if is_valid then
    ...
    let get_con id =
      if id < 0 then
        Connections.find cons (Utils.FD.of_int (-id))
      else
  ...
  let from_file store cons doms file =
    ...
    let channel = open_in file in
      ...
      (fun () -> from_channel store doms cons channel)
  ...
  let to_channel store cons (rw, evtchn) chan =
    ...
    Store.dump_fct store (fun path node ->
        ...
        let fullpath =
          Store.Path.to_string (Store.Path.of_path_and_name path name)
        in
        let permstr = Perms.Node.to_string perms in
        fprintf chan "store,%s,%s,%s\n" (hexify fullpath) (hexify permstr)
          (hexify value)
    ) ;
    flush chan ;
    ()
  ...
  let to_file store cons fds file =
    let channel =
      open_out_gen [Open_wronly; Open_creat; Open_trunc] 0o600 file
    in
    finally
      (fun () -> to_channel store cons fds channel)
...
let main () =
  ...
  (try Unixext.mkdir_rec (Filename.dirname pidfile) 0o755 with _ -> ()) ;
  ...
  let advance_next_frequent_ops () =
    next_frequent_ops :=
      Unix.gettimeofday () +. !Define.conflict_max_history_seconds
  in
  ...
  List.iter
    (fun path -> Store.write store Perms.Connection.full_rights path "")
    Store.Path.specials ;

  let rw_sock, domains =
    if cf.restart && Sys.file_exists Disk.xs_daemon_database then (
      let rw, domains =
        DB.from_file store domains_init cons Disk.xs_daemon_database
      in
  ...
  ( if cf.activate_access_log then
      let post_rotate () =
        DB.to_file store cons (None, eventchn) Disk.xs_daemon_database
      in
  ...
  let periodic_ops now =
    ...
    if
      !ring_scan_interval >= 0
      && now > !last_scan_time +. float !ring_scan_interval
    then (
      last_scan_time := now ;
      Domains.iter domains ring_scan_checker
    ) ;

    (* make sure we don't print general stats faster than 2 min *)
    if now > !last_stat_time +. 120. then (
      info "Transaction conflict statistics for last %F seconds:"
        (now -. !last_stat_time) ;
      last_stat_time := now ;
      Domains.iter domains
        (Domain.log_and_reset_conflict_stats (info "Dom%d caused %Ld conflicts")) ;
      info "%Ld failed transactions; of these no culprit was found for %Ld"
        !Transaction.failed_commits
        !Transaction.failed_commits_no_culprit ;
      Transaction.reset_conflict_stats () ;

      let gc = Gc.stat () in
      let lanon, lanon_ops, lanon_watchs, ldom, ldom_ops, ldom_watchs =
        Connections.stats cons
      in
      let store_nodes, store_abort, store_coalesce = Store.stats store in
      let symtbl_len, symtbl_entries = Symbol.stats () in

      info "store stat: nodes(%d) t-abort(%d) t-coalesce(%d)" store_nodes
        store_abort store_coalesce ;
      info "sytbl stat: length(%d) entries(%d)" symtbl_len symtbl_entries ;
      info "  con stat: anonymous(%d, %d o, %d w) domains(%d, %d o, %d w)" lanon
        lanon_ops lanon_watchs ldom ldom_ops ldom_watchs ;
      info
        "  mem stat: minor(%.0f) promoted(%.0f) major(%.0f) heap(%d w, %d c) \
         live(%d w, %d b) free(%d w, %d b)"
        gc.Gc.minor_words gc.Gc.promoted_words gc.Gc.major_words
        gc.Gc.heap_words gc.Gc.heap_chunks gc.Gc.live_words gc.Gc.live_blocks
    ...
    ( if !quit then
        ...
        | domains ->
            List.iter
              (fun con ->
                warn "%s prevents live update" (Connection.get_domstr con)
              )
              domains
    ) ;
    ...
    List.iter
      (fun c ->
        ...
        | None ->
            ()
        | Some d ->
            Domain.incr_io_credit d
      )
      peaceful_mw ;
    let start_time = Unix.gettimeofday () in
    let timeout =
      let until_next_activity =
        if Domains.all_at_max_credit domains then
          period_ops_interval
        else
          min (max 0. (!next_frequent_ops -. start_time)) period_ops_interval
      in
