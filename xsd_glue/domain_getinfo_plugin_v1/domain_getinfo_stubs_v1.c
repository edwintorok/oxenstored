...
static inline xc_interface *
xsd_glue_xch_of_val (value v)
{
...
static void
xsd_glue_xenctrl_finalize (value v)
{
...
#define ERR (error && error->code != XC_ERROR_NONE)
        ...
        int ret = asprintf (&str,
                            ...
                            ERR ? xc_error_code_to_desc (error->code) :
                            strerror (errno),
                            ERR ? ": " : "",
                            ERR ? error->message : "",
                            func, line);
...
CAMLprim value
stub_xsd_glue_xc_interface_open (value unit)
{
        CAMLparam1 (unit);
        CAMLlocal1 (result);
        xc_interface *xch;

        result = caml_alloc_custom (&xsd_glue_xenctrl_ops, sizeof (xch), 0,
                                    1);
...
xsd_glue_alloc_domaininfo (const xc_domaininfo_t * info)
{
        ...
        Store_field (result, 3,
                     Val_int (MASK_EXTR
                              (info->flags, XEN_DOMINF_shutdownmask)));

        CAMLreturn (result);
}

CAMLprim value
stub_xsd_glue_xc_domain_getinfo (value xch_val, value domid)
{
...
CAMLprim value
stub_xsd_glue_xc_domain_getinfolist (value xch_val)
{
        CAMLparam1 (xch_val);
        CAMLlocal1 (result);
        xc_interface *xch = xsd_glue_xch_of_val (xch_val);
        ...
        for (i = 0; i < retval; i++)
          {
                  caml_modify (&Field (result, i),
                               xsd_glue_alloc_domaininfo (info + i));
          }
